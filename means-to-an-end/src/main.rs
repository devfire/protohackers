...
fn insert_data(timestamp: &i32, price: &i32) {
    unimplemented!()
}

fn query_data(time1: &i32, time2: &i32) {
    unimplemented!()
}
...
async fn main() -> io::Result<()> {
    ...
    let create_table_query =
        "CREATE TABLE messages (msg_type INTEGER, begin_time INTEGER, end_time INTEGER)";

    info!("Creating the table.");

    let connection = sqlite::open(":memory:").expect("Failed to open sqlite");
